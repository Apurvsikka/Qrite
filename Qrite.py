...
import cv2
#this imports webbrowser display
import webbrowser
...
def close_window():
    ...
    if askyesno(title='Close Qrite', message='Are you sure you want to close the application?'):
...
tab_control.add(third_tab, text='Credits')
...
# creates the canvas for containing all the widgets in the third tab
third_canvas = Canvas(third_tab, width=500, height=480)
# packing the canvas to the third tab
third_canvas.pack()
...
'''Below are the widgets for the third tab'''
credits_label = ttk.Label(window, text='Qrite v1.0.0\n\n')

third_canvas.create_window(250, 50, window=credits_label)

credits_label2 = ttk.Label(window, text='Qrite is a simple QRCode generator and detector\n\n')
third_canvas.create_window(250, 100, window=credits_label2)
# creating the credits label
credits_label = ttk.Label(window, text='Qrite v1.0.0\n\n')

credits_label2 = ttk.Label(window, text='Qrite is a simple QRCode generator and detector\n\n')

credits_label3 = ttk.Label(window, text='DEVELOPERS\n\n')
credits_label4 = ttk.Label(window, text='->Apurv Sikka\n\n') 
third_canvas.create_window(250, 150, window=credits_label3)
third_canvas.create_window(250, 200, window=credits_label4)


def callback(url):
   webbrowser.open_new_tab(url)
#llolo
social = ttk.Label(window,text='social')
third_canvas.create_window(250,250, window=social)

#Create a Label to display the link
link = ttk.Label(window, text="🇬‌🇮‌🇹‌🇭‌🇺‌🇧‌",font=('Helveticabold', 8))
link.pack()
link.bind("<Button-1>", lambda e:
callback("http://www.github.com/Apurvsikka"))
third_canvas.create_window(250,300, window=link)
